...
function clockIn($staff_id) {
   ...
   /* $stmt = mysqli_prepare($conn, "INSERT INTO tblattendance (staff_id, time_in, date) VALUES (?, ?, ?)");
    ...
    $stmt = mysqli_prepare($conn,  "INSERT INTO tblattendance (staff_id, time_in, date, status) VALUES (?, ?, ?, ?)");
    $status = 'In';
    mysqli_stmt_bind_param($stmt, 'ssss', $staff_id, $currentTime, $currentDate, $status);
    $result = mysqli_stmt_execute($stmt);
