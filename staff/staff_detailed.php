...
// Check if the user has the role of Staff
...
$get_id = isset($_GET['id']) ? intval($_GET['id']) : null;
$session_id = intval($_SESSION['slogin']);

// If staff is trying to view someone else's profile, redirect to their own
if ($userRole === 'Staff' && $get_id != $session_id) {
    header('Location: staff_detailed.php?id=' . urlencode($session_id) . '&view=2');
    exit();
}
?>
